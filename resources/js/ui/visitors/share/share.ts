import {Facebook} from './../../../../../node_modules/goodshare.js/src/providers/Facebook'
import {Twitter} from './../../../../../node_modules/goodshare.js/src/providers/Twitter'
import {WhatsApp} from './../../../../../node_modules/goodshare.js/src/providers/WhatsApp'
import {appUrl, blogPostRelativeUrl} from "../../../utils/constants";

const shareMessagePrefix = "Checkout this blog by Renee Tikolo:";

export class Share {
    clickedShareAnchor: HTMLAnchorElement;

    setupShareAnchors() {
        $(document).on('click', '.share-blog', e => {
            e.preventDefault();
            this.clickedShareAnchor = e.currentTarget;
            const popover = new Popover(e.currentTarget);
            popover.show();
            this.setupPopoverSocialShare();
        })
    }

    private setupPopoverSocialShare() {
        const url = `${appUrl}/${blogPostRelativeUrl}/${this.getClickedShareAnchorBlogId()}`;
        const shareMessage = `${shareMessagePrefix} ${this.getClickedShareAnchorTitle()}`;
    ...
    private getClickedShareAnchorBlogId(): number {
        return parseInt(this.clickedShareAnchor.dataset.blogId);
    }

    private getClickedShareAnchorTitle(): string {
...
class Popover {
    ...
    constructor(shareAnchor: HTMLAnchorElement) {
